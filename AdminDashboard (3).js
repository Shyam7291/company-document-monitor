...
const Icon = ({ name, size = 20, strokeWidth = 1.7 }) => {
  const paths = {
    ...
    /* ---- bottom navigation icon set (matches reference design) ---- */
    cubes: <>
    ...
    homeFill: <>
      <path
        fill="currentColor"
        stroke="none"
        d="M12 3.05a1.4 1.4 0 0 1 .9.33l7.55 6.36a1 1 0 0 1-.64 1.77h-1.4v7.16A1.83 1.83 0 0 1 16.58 20.5h-2.44v-3.9a2.14 2.14 0 0 0-4.28 0v3.9H7.42a1.83 1.83 0 0 1-1.83-1.83v-7.16h-1.4a1 1 0 0 1-.64-1.77l7.55-6.36a1.4 1.4 0 0 1 .9-.33z"
      />
    </>,
    doc: <>
...
export default function AdminDashboard({
  ...
  const navClick = key => {
    ...
    if (key === "Bidding") {
      if (onTransporterBidding) {
        onTransporterBidding();
      } else {
        showToast("Opening Transporter Bidding");
      }
    ...
    if (key === "Requests") {
      if (onRateRequests) {
        ...
        showToast("Opening Rate Requests");
      }
    ...
    if (key === "Orders") {
      if (onConfirmedOrders) {
        onConfirmedOrders();
      } else {
        showToast("Opening Confirmed Orders");
      }
    }
  };

  /* five-item bottom navigation — matches the reference design */
  const navItems = [
    { key: "Samples", label: "Samples", icon: "cubes" },
    { key: "Bidding", label: "Transport", icon: "gavel", dot: true },
    { key: "Home", label: "Home", icon: "homeFill", center: true },
    { key: "Requests", label: "Rate Req.", icon: "doc" },
    { key: "Orders", label: "Orders", icon: "clipboardCheck" }
  ];

  const activeIndex = Math.max(0, navItems.findIndex(n => n.key === activeNav));
  ...
  return (
    <div className="nx-admin">
      ...
      {/* ---------------- Bottom navigation (reference design) ---------------- */}
      <div className="nx-navwrap">
        <div className="nx-navdock">
        <span className="nx-nav-cradle" aria-hidden="true" />
        <nav className="nx-bottom" aria-label="Admin primary navigation">
          <span className="nx-bottom-sheen" aria-hidden="true" />
            ...
            if (item.center) {
              return (
                <button
                  key={item.key}
                  type="button"
                  className={"nx-nav-item nx-nav-center" + (isActive ? " active" : "")}
                  aria-current={isActive ? "page" : undefined}
                  onClick={() => navClick(item.key)}
                >
                  <span className="nx-center-orb">
                    <span className="nx-center-halo" aria-hidden="true" />
                    <span className="nx-center-inner">
                      <span className="nx-center-icon">
                        <Icon name={item.icon} size={22} strokeWidth={0} />
                      </span>
                      <span className="nx-center-label">{item.label}</span>
                    </span>
                  </span>
                </button>
              );
            ...
            return (
              ...
              >
                <span className="nx-nav-icon">
                  ...
                  {item.dot ? <i className="nx-nav-notice" aria-hidden="true" /> : null}
                </span>
          ...
          <span className="nx-nav-dots" aria-hidden="true">
            {navItems.map((n, i) => (
              <i key={n.key} className={"nx-nav-dot" + (i === activeIndex ? " on" : "")} />
            ))}
          </span>
        </nav>
        </div>
...
.nx-admin{
  ...
  background:#f4f7fb;box-sizing:border-box;overflow-x:hidden;padding-bottom:126px;
...
/* ================================================================== *
 * BOTTOM NAVIGATION — dark glass dock, 5 items, raised glowing
 * centre "Home" orb sitting in a circular cradle (reference design)
 * ================================================================== */
.nx-navwrap{
...
/* dock wrapper — holds the cradle behind and the bar in front */
.nx-navdock{
  position:relative;
  width:min(100%,520px);
  height:86px;
...
/* circular hump behind the centre orb (same tint as the bar) */
.nx-nav-cradle{
  position:absolute;z-index:0;
  left:50%;top:50%;transform:translate(-50%,-50%);
  width:98px;height:98px;border-radius:50%;
  background:linear-gradient(180deg,rgba(24,33,52,.93),rgba(16,23,38,.94));
  border:1px solid rgba(255,255,255,.13);
  box-shadow:0 16px 34px rgba(8,14,28,.22);
...
.nx-bottom{
  ...
  border-radius:28px;
  background:
    linear-gradient(105deg,rgba(70,120,225,.22) 0%,rgba(28,38,58,0) 40%,rgba(28,38,58,0) 60%,rgba(228,106,24,.26) 100%),
    linear-gradient(180deg,rgba(27,36,56,.92),rgba(16,23,38,.94));
  -webkit-backdrop-filter:blur(20px) saturate(1.4);
  backdrop-filter:blur(20px) saturate(1.4);
  box-shadow:
    0 22px 46px rgba(8,14,28,.30),
    0 4px 14px rgba(8,14,28,.16),
    inset 0 1px 0 rgba(255,255,255,.10);
}

/* subtle blue→orange sheen across the bar */
.nx-bottom-sheen{
  position:absolute;inset:0;border-radius:inherit;pointer-events:none;
  background:linear-gradient(105deg,rgba(80,130,235,.10) 0%,transparent 42%,transparent 58%,rgba(255,140,40,.12) 100%);
}

/* ---- side nav items ---- */
.nx-admin .nx-nav-item{
  ...
  font-size:12px;font-weight:500;letter-spacing:0;line-height:1;
  cursor:pointer;
...
.nx-nav-icon{
  ...
  transition:transform .26s cubic-bezier(.25,1.35,.5,1),color .22s ease;
}
.nx-nav-label{
...
.nx-admin .nx-nav-item:hover{color:#ffb066}
.nx-nav-item:hover .nx-nav-icon{transform:translateY(-2px)}
.nx-admin .nx-nav-item.active{color:#ff9a3d}
.nx-admin .nx-nav-item.active .nx-nav-icon{color:#ff9a3d}
.nx-nav-item:active{transform:scale(.95)}
.nx-nav-item:focus-visible{outline:2px solid rgba(255,166,70,.6);outline-offset:-3px;border-radius:20px}

/* notification dot on the Transport icon */
.nx-nav-notice{
  position:absolute;top:-3px;right:-4px;
  width:8px;height:8px;border-radius:50%;
  background:#f97316;
  box-shadow:0 0 0 2px rgba(19,27,44,.95),0 2px 6px rgba(249,115,22,.6);
}

/* ---- centre raised glowing orb ---- */
.nx-nav-center{padding:0;overflow:visible}
.nx-center-orb{
  position:absolute;
  left:50%;top:5px;transform:translateX(-50%);
  width:60px;height:60px;
  display:grid;place-items:center;border-radius:50%;
  transition:transform .3s cubic-bezier(.25,1.35,.5,1);
}
.nx-center-halo{
  position:absolute;inset:-20px;border-radius:50%;
  background:radial-gradient(circle,rgba(255,166,70,.60) 0%,rgba(255,140,40,.30) 40%,rgba(255,130,30,0) 70%);
  filter:blur(9px);
  animation:nxGlow 3.2s ease-in-out infinite;
}
.nx-center-inner{
  position:relative;
  ...
  background:radial-gradient(circle at 50% 36%,#fffdf7 0%,#fff1dd 34%,#ffdcb4 62%,#ffc590 84%,#ffb579 100%);
  box-shadow:
    0 0 0 1px rgba(255,201,146,.9),
    0 0 16px 4px rgba(255,150,50,.5),
    0 10px 22px rgba(170,66,8,.34),
    inset 0 -6px 13px rgba(255,168,86,.5),
    inset 0 2px 6px rgba(255,255,255,.95);
  transition:box-shadow .24s ease,transform .24s ease;
}
.nx-admin .nx-center-icon{
  display:grid;place-items:center;
  color:#f2760c;line-height:0;
}
.nx-admin .nx-center-label{
  font-size:10px;font-weight:700;letter-spacing:.2px;line-height:1;
  color:#ef7108;
}
.nx-nav-center:hover .nx-center-orb{transform:translateX(-50%) translateY(-2px)}
.nx-nav-center:active .nx-center-orb{transform:translateX(-50%) scale(.95)}
.nx-nav-center.active .nx-center-inner{
  box-shadow:
    0 0 0 1px rgba(255,201,146,1),
    0 0 22px 6px rgba(255,150,50,.62),
    0 12px 26px rgba(170,66,8,.38),
    inset 0 -6px 14px rgba(255,168,86,.55),
    inset 0 2px 6px rgba(255,255,255,.98);
}
.nx-nav-center:focus-visible{outline:none}
.nx-nav-center:focus-visible .nx-center-inner{outline:2px solid rgba(255,255,255,.75);outline-offset:3px}

/* ---- page indicator dots at the bottom edge ---- */
.nx-nav-dots{
  position:absolute;z-index:2;left:50%;bottom:7px;transform:translateX(-50%);
  display:flex;align-items:center;gap:5px;pointer-events:none;
}
.nx-nav-dot{
  width:4px;height:4px;border-radius:3px;
  background:rgba(255,255,255,.28);
  transition:width .28s cubic-bezier(.25,1.35,.5,1),background .22s ease;
}
.nx-nav-dot.on{width:14px;background:#f97316;box-shadow:0 0 6px rgba(249,115,22,.6)}

@media (max-width:390px){
  .nx-navdock{height:82px}
  .nx-bottom{border-radius:26px}
  .nx-nav-cradle{width:92px;height:92px}
  .nx-admin .nx-nav-item{gap:7px;padding-top:18px;font-size:11px}
  .nx-nav-icon{width:22px;height:22px}
  .nx-center-orb,.nx-center-inner{width:56px;height:56px}
  .nx-center-label{font-size:9.5px}
}
...
.nx-toast{position:fixed;z-index:80;left:50%;bottom:110px;transform:translateX(-50%);display:flex;
...
@keyframes nxGlow{0%,100%{opacity:.75;transform:scale(1)}50%{opacity:1;transform:scale(1.06)}}
...
@media(min-width:700px){
  ...
  .nx-navwrap{padding-bottom:calc(16px + env(safe-area-inset-bottom,0px))}
