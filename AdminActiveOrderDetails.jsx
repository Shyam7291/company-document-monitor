...
export default function AdminActiveOrderDetails({ onBack, onUpdated }) {
  const [now, setNow] = useState(Date.now());
  const [openStage, setOpenStage] = useState("LOADING");
  const [orderStatus, setOrderStatus] = useState("LOADING");
  const [loadingDate, setLoadingDate] = useState(ORDER.loadingDate);
  ...
  const [activity, setActivity] = useState([
    { time: ago(90), text: "Loading started" },
    { time: ago(45), text: "Vehicle 1 loaded" },
    { time: ago(25), text: "Vehicle 2 loaded" }
  ]);
  ...
  const totals = useMemo(() => {
    const loadedQty = vehicles.reduce((sum, v) => sum + (v.status === "LOADED" ? Number(v.loadedQty || 0) : 0), 0);
    const loadedVehicles = vehicles.filter(v => v.status === "LOADED").length;
    const byMaterial = ORDER.materials.map(m => ({ ...m, loaded: vehicles.filter(v => v.materialId === m.id && v.status === "LOADED").reduce((sum, v) => sum + Number(v.loadedQty || 0), 0) }));
    return { loadedQty, loadedVehicles, byMaterial, totalQty: ORDER.materials.reduce((s, m) => s + m.quantity, 0), orderValue: ORDER.materials.reduce((s, m) => s + m.quantity * m.finalRate, 0) };
  }, [vehicles]);
  ...
  const currentRank = stageRank[orderStatus] ?? 0;
  ...
  const saveVehicle = data => {
    ...
    setVehicles(list => list.map(v => v.id === data.id ? { ...v, ...data, number: formattedNumber, driver: String(data.driver).trim(), phone: mobile, expectedQty: expected } : v));
    setVehicleForm(null);
    setFormErrors({});
    notify("Vehicle details saved");
  };

  const updateVehicleStatus = () => {
    const becameLoaded = statusEditor.status === "LOADED" && statusEditor.vehicle.status !== "LOADED";
    setVehicles(list => list.map(v => v.id === statusEditor.vehicle.id
      ? { ...v, status: statusEditor.status, loadedQty: statusEditor.status === "LOADED" ? Number(statusEditor.loadedQty || v.expectedQty) : 0, loadedAt: statusEditor.status === "LOADED" ? new Date().toISOString() : null }
      : v));
    if (becameLoaded) {
      const vehicleIndex = vehicles.findIndex(v => v.id === statusEditor.vehicle.id) + 1;
      log(`Vehicle ${vehicleIndex} loaded`);
    }
    setStatusEditor(null);
    notify("Vehicle status updated");
  };

  const dispatch = () => { setOrderStatus("IN_TRANSIT"); setOpenStage("IN_TRANSIT"); setDispatchConfirm(false); log("All vehicles dispatched"); notify("Order is now In Transit"); onUpdated?.({ status: "IN_TRANSIT", vehicles }); };
  const reopenLoading = () => { if (!reason.trim()) return notify("Correction reason is required"); setOrderStatus("LOADING"); setOpenStage("LOADING"); setReverseDispatch(false); log("Loading reopened"); setReason(""); notify("Loading stage reopened"); };
  ...
  return <div className="od-root">
    ...
    <header className="od-header">
      <div className="od-width">
        ...
        <div className="od-id">
          ...
          <em className={`status-${orderStatus.toLowerCase()}`}><i/>{orderStatus.replace("_", " ")}</em>
        </div>
    ...
    <main className="od-width od-main">
      ...
      <section className="od-panel od-overview">
        ...
        <div className="od-materials">{ORDER.materials.map(m => <button type="button" key={m.id} onClick={() => setMaterialDetail(m)}>
          <MaterialArt material={m}/>
          <span><b>{m.name}</b><small>{m.quantity} t · Supplier: {m.seller}</small></span>
          <strong>{money(m.finalRate)}/t</strong>
        </button>)}</div>
      ...
      <section className="od-stages">{stages.map((stage, index) => {
        ...
        return <article className={`od-stage ${completeStage ? "complete" : ""} ${locked ? "locked" : ""} ${open ? "open" : ""}`} key={stage.id}>
          ...
          {open && stage.id === "LOADING" && <div className="od-stage-body">
            ...
            <button className="od-dispatch" disabled={!allLoaded} onClick={() => setDispatchConfirm(true)}>{allLoaded ? "All Loaded · Dispatch Order" : `${totals.totalQty - totals.loadedQty} t Remaining`}</button>
          </div>}
      ...
      <section className="od-panel od-activity">
        <div className="od-head"><Icon name="history" size={15}/><b>Activity Log</b><button onClick={() => setIssueModal(true)}>Report Issue</button></div>
    ...
    {materialDetail && <div className="od-overlay" onMouseDown={() => setMaterialDetail(null)}>
      <section className="od-sheet od-material-sheet" onMouseDown={e => e.stopPropagation()}>
        ...
        <div className="od-material-large"><MaterialArt material={materialDetail}/></div>
        <dl>
        ...
        <div className="od-material-actions">
          <button type="button" onClick={() => copy(materialDetail.sellerPhone, "Supplier number")}><Icon name="copy" size={14}/>Copy Number</button>
          <a href={`tel:${materialDetail.sellerPhone.replace(/\s/g, "")}`}><Icon name="phone" size={14}/>Call Supplier</a>
        </div>
      </section>
    </div>}
...
.od-materials>button{width:100%;display:grid;grid-template-columns:52px 1fr auto;align-items:center;gap:10px;padding:8px;
  border:1px solid var(--line);border-radius:13px;background:linear-gradient(180deg,#fff,#f7f9fc);text-align:left;color:var(--ink)}
...
.od-add-vehicle{width:100%;margin-top:10px;min-height:40px;color:var(--o2);border-style:dashed;border-color:rgba(249,115,22,.45);background:rgba(249,115,22,.05)}
.od-add-vehicle:hover{background:rgba(249,115,22,.11);border-color:var(--o)}
.od-dispatch{width:100%;margin-top:12px;padding:14px;border:0;border-radius:13px;color:#fff;
