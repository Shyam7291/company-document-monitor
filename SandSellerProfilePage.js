...
import {
  ...
  Mail, BadgeCheck, Hash, ArrowUpRight, Star,
} from "lucide-react";
import {
  getCurrentSeller,
  getSellerProfile,
...
/**
 * SandSellerProfilePage — redesigned edition.
...
function Ring({ value, size = 104, stroke = 4, light = false, children }) {
  ...
  return <div className={`ssp-ring ${light ? "is-light" : ""}`} style={{ width: size, height: size }}>
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} aria-hidden="true">
      ...
      <circle className="ssp-ring-fill" cx={center} cy={center} r={radius} strokeWidth={stroke}
...
export default function SandSellerProfilePage({
  ...
  return <div className="sand-seller-profile">
    ...
    <main className="ssp-content" aria-busy={loading}>
      ...
      <section className="ssp-hero ssp-reveal" aria-labelledby="ssp-profile-title">
        ...
        <div className="ssp-hero-top">
          <span className="ssp-eyebrow"><Star size={10}/>YOUR SELLER ACCOUNT</span>
      ...
      {/* ─── Quick actions ────────────────────────────────────── */}
      <div className="ssp-actions ssp-reveal" style={{ "--delay": "80ms" }}>
        <button type="button" className="ssp-action is-indigo" disabled={!ready} onClick={openEdit}>
          <i><SquarePen size={20}/></i><b>Edit details</b><span>Plant, contact & address</span><ArrowUpRight size={15} className="ssp-action-arrow"/>
        </button>
        <button type="button" className="ssp-action is-cyan" disabled={!ready} onClick={() => photoInput.current?.click()}>
          <i><Camera size={20}/></i><b>Change photo</b><span>JPG, PNG or WEBP</span><ArrowUpRight size={15} className="ssp-action-arrow"/>
        </button>
        <button type="button" className={`ssp-action ${profile.aadhaarVerified ? "is-green" : "is-violet"}`} disabled={!ready || profile.aadhaarVerified}
          onClick={() => { setError(""); setSheet("aadhaar"); }}>
          <i><Fingerprint size={20}/></i><b>{profile.aadhaarVerified ? "Aadhaar verified" : "Verify Aadhaar"}</b>
          <span>{profile.aadhaarVerified ? "Identity confirmed" : "Secure identity check"}</span>
          {profile.aadhaarVerified ? <CheckCircle2 size={15} className="ssp-action-arrow"/> : <ArrowUpRight size={15} className="ssp-action-arrow"/>}
        </button>
      </div>
    ...
    {sheet === "otp" && pendingChange && <Sheet title="Secure mobile change" subtitle="One OTP for each number. One safer account." busy={busy}
      ...
      <div className="ssp-otp-intro"><i><LockKeyhole size={22}/></i><div><b>Verify both mobile numbers</b><p>Separate codes have been sent to your current and new mobile numbers.</p></div></div>
...
/* Quick actions */
.ssp-actions { display:grid; grid-template-columns:repeat(3,1fr); gap:10px; margin-top:16px; }
.ssp-action { position:relative; display:grid; justify-items:start; gap:2px; min-width:0; padding:14px 13px 13px; border:1px solid rgba(255,255,255,.95); border-radius:20px; background:var(--ssp-glass); color:var(--ssp-ink); text-align:left; box-shadow:var(--ssp-shadow); backdrop-filter:blur(14px); overflow:hidden; }
.ssp-action::before { content:""; position:absolute; inset:0; opacity:0; transition:opacity .25s; background:var(--ssp-grad-soft); }
.ssp-action:hover:not(:disabled) { transform:translateY(-3px); box-shadow:var(--ssp-shadow-hover); }
.ssp-action:hover:not(:disabled)::before { opacity:1; }
.ssp-action>* { position:relative; z-index:1; }
.ssp-action i { display:grid; place-items:center; width:40px; height:40px; margin-bottom:8px; border-radius:13px; color:#fff; box-shadow:0 8px 18px rgba(91,108,255,.25); }
.ssp-action.is-indigo i { background:linear-gradient(135deg,#5b6cff,#7d8cff); }
.ssp-action.is-cyan i { background:linear-gradient(135deg,#22c1ee,#4fd6f5); box-shadow:0 8px 18px rgba(34,193,238,.28); }
.ssp-action.is-violet i { background:linear-gradient(135deg,#8b5cf6,#a985ff); box-shadow:0 8px 18px rgba(139,92,246,.28); }
.ssp-action.is-green i { background:linear-gradient(135deg,#16b981,#3dd39d); box-shadow:0 8px 18px rgba(22,185,129,.26); }
.ssp-action.is-green { opacity:1!important; cursor:default; }
.ssp-action b { font-size:12px; font-weight:800; letter-spacing:-.2px; line-height:1.25; }
.ssp-action span { font-size:9.5px; color:var(--ssp-muted); line-height:1.4; }
.ssp-action-arrow { position:absolute; right:12px; top:13px; color:#a2abc9; transition:transform .25s,color .25s; }
.ssp-action:hover:not(:disabled) .ssp-action-arrow { color:var(--ssp-indigo); transform:translate(2px,-2px); }
.ssp-action.is-green .ssp-action-arrow { color:var(--ssp-green); }
...
.ssp-phone-warning { display:flex; align-items:flex-start; gap:10px; padding:13px; border:1px solid rgba(245,165,36,.23); border-radius:15px; background:linear-gradient(135deg,#fff8e8,#fffdf6); color:#9a6a23; }
...
/* OTP */
.ssp-otp-intro { display:flex; align-items:center; gap:12px; padding:15px; margin-bottom:18px; border:1px solid #e3e7fa; border-radius:19px; background:linear-gradient(135deg,#eef1ff,#f6eeff); }
.ssp-otp-intro>i { flex:none; display:grid; place-items:center; width:44px; height:44px; border-radius:14px; background:var(--ssp-grad); color:#fff; box-shadow:0 8px 18px rgba(91,108,255,.26); }
.ssp-otp-intro b { color:#424b7a; font-size:12px; }
.ssp-otp-intro p { margin:4px 0 0; color:#727b99; font-size:11px; }
.ssp-otp-card { padding:15px; margin-top:12px; border:1px solid var(--ssp-line); border-radius:19px; background:#fff; box-shadow:0 6px 18px rgba(64,84,150,.05); }
...
.ssp-otp-input { width:100%; min-height:54px; padding:10px 12px; margin-top:13px; border:1px solid #dfe5f5; border-radius:13px; background:#f9faff; color:#4651a1; text-align:center; font-size:24px!important; font-weight:750; letter-spacing:.35em; transition:border-color .2s,box-shadow .2s; }
.ssp-otp-input:focus { border-color:#919df3; box-shadow:0 0 0 4px rgba(91,108,255,.10); background:#fff; }
...
@media(min-width:760px) {
  ...
  .ssp-actions { gap:14px; margin-top:20px; }
  .ssp-action { padding:18px 16px 16px; }
  .ssp-action b { font-size:13.5px; }
  .ssp-action span { font-size:10.5px; }
...
@media(max-width:390px) {
  ...
  .ssp-actions { gap:8px; }
  .ssp-action { padding:12px 10px 11px; border-radius:17px; }
  .ssp-action i { width:34px; height:34px; }
  .ssp-action b { font-size:11px; }
  .ssp-action span { font-size:9px; }
  .ssp-action-arrow { right:9px; top:10px; }
