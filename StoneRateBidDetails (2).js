...
.dt-bid-btn.submitted{background:#e9edf4;color:#7a8599;border:1px solid #dfe4ee;box-shadow:none;cursor:not-allowed}
.dt-bid-btn.submitted:after{display:none}

/* ---------- Your position banner ---------- */
.dt-position{position:relative;overflow:hidden;display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:12px;margin-top:14px;padding:14px 16px;border-radius:20px;color:#fff;background:linear-gradient(135deg,#2457e6,#6c4ee7 58%,#00a9c7);box-shadow:0 14px 30px rgba(75,84,194,.28);animation:dt-rise .45s ease both}
.dt-position:after{content:"";position:absolute;width:150px;height:150px;border-radius:50%;right:-60px;top:-80px;background:rgba(255,255,255,.14)}
.dt-position-icon{position:relative;z-index:1;width:42px;height:42px;border-radius:14px;display:grid;place-items:center;background:rgba(255,255,255,.18);border:1px solid rgba(255,255,255,.28)}
.dt-position-text,.dt-position-total{position:relative;z-index:1}
.dt-position small{display:block;font-size:9px;letter-spacing:1px;text-transform:uppercase;font-weight:800;opacity:.8}
.dt-position-text b{display:block;font-size:26px;font-weight:900;line-height:1.1;margin-top:3px;letter-spacing:-.5px}
.dt-position-total{text-align:right}
.dt-position-total b{display:block;font-size:15px;font-weight:900;margin-top:4px;white-space:nowrap}
...
export default function StoneRateBidDetails({ bid = BID, onBack = () => {}, onRefresh = () => {}, onBidSubmitted = () => {} }) {
  ...
  const [submitted, setSubmitted] = useState(null); // { total } once the bid is submitted
  ...
  // Position among all bidders: lower total ranks higher (ties keep the earlier bidder ahead).
  const myPosition = submitted ? TOP.filter(r => r.total <= submitted.total).length + 1 : null;

  const finalSubmit = () => {
    const total = transport + permit;
    onBidSubmitted({ bid, transportationCost: transport, permitCost: permit, total, vehicles });
    setSubmitted({ total });
    setStage("success");
  };
  const openSheet = () => {
  ...
  return (
    <div className="dt-app">
      ...
      <main className="dt-shell dt-main">
        ...
        {/* Your position (only after a successful submission) */}
        {submitted && (
          <section className="dt-position" role="status">
            <div className="dt-position-icon"><BadgeCheck size={20} /></div>
            <div className="dt-position-text">
              <small>Your position in the bidding</small>
              <b>#{myPosition}</b>
            </div>
            <div className="dt-position-total">
              <small>Your bid</small>
              <b>{money(submitted.total)}</b>
            </div>
          </section>
        )}
        ...
        <div className="dt-cta">
          {submitted ? (
            <button className="dt-bid-btn submitted" disabled aria-disabled="true">
              <CheckCircle2 size={16} />Bid submitted already
            </button>
          ) : (
