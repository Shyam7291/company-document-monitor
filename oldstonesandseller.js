...
html,body,#root{margin:0;min-height:100%;font-family:"Plus Jakarta Sans",Inter,"Segoe UI",system-ui,sans-serif;background:#f4f7fd;color:#1b2340;-webkit-font-smoothing:antialiased}
button{font:inherit;cursor:pointer}
.app{--ink:#1b2340;--muted:#6b7590;--line:rgba(120,135,180,.16);--indigo:#5b6cff;--violet:#8b5cf6;--cyan:#22c1ee;--mint:#22c58f;--amber:#f5a524;--rose:#ff6b8a;--grad:linear-gradient(135deg,#5b6cff 0%,#8b5cf6 55%,#22c1ee 100%);--card:rgba(255,255,255,.82);--shadow:0 12px 34px rgba(64,84,150,.10),0 2px 6px rgba(64,84,150,.05);position:relative;min-height:100dvh;padding-bottom:110px;overflow-x:hidden;background:linear-gradient(180deg,#f7f9ff 0%,#f3f6fd 45%,#eef3fb 100%)}
.bg-orb{position:fixed;border-radius:50%;filter:blur(70px);opacity:.55;pointer-events:none;z-index:0;animation:float 14s ease-in-out infinite alternate}
.o1{width:320px;height:320px;left:-120px;top:-80px;background:radial-gradient(circle,#c7d0ff,transparent 70%)}
.o2{width:360px;height:360px;right:-140px;top:180px;background:radial-gradient(circle,#c3f4ff,transparent 70%);animation-delay:-5s}
.o3{width:300px;height:300px;left:30%;bottom:-120px;background:radial-gradient(circle,#d9f9ec,transparent 70%);animation-delay:-9s}
.app button:focus-visible{outline:3px solid rgba(91,108,255,.35);outline-offset:2px}
.top{position:sticky;top:0;z-index:40;height:66px;max-width:1180px;margin:auto;padding:0 12px;display:grid;grid-template-columns:minmax(0,1fr) auto minmax(0,1fr);align-items:center;gap:8px;background:rgba(255,255,255,.72);backdrop-filter:blur(22px) saturate(160%);-webkit-backdrop-filter:blur(22px) saturate(160%);border-bottom:1px solid var(--line)}
.top>div{min-width:0;display:flex;align-items:center;gap:10px}
...
.hello{min-width:0}.hello span,.hello b{display:block}
.hello span{font-size:10px;color:var(--muted);letter-spacing:.2px}
.hello b{max-width:110px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px;color:var(--ink);font-weight:800}
.brand{display:flex;align-items:center;gap:5px;font-size:17px;font-weight:900;letter-spacing:-.6px;white-space:nowrap}
.brand i{width:24px;height:24px;display:grid;place-items:center;border-radius:8px;background:var(--grad);color:#fff;box-shadow:0 6px 14px rgba(91,108,255,.35)}
...
.hero-card{position:relative;overflow:hidden;display:flex;align-items:center;justify-content:space-between;gap:14px;padding:20px 18px;border-radius:24px;background:var(--grad);color:#fff;box-shadow:0 18px 40px rgba(91,108,255,.30)}
...
.material:hover,.metric:hover{transform:translateY(-4px);box-shadow:0 20px 44px rgba(64,84,150,.16)}
...
.material p{display:flex;align-items:center;gap:5px;margin:10px 0 0;padding:5px 9px;width:max-content;border-radius:99px;font-size:10px;font-weight:700;background:rgba(27,35,64,.05);color:#3b4468}
...
.material.c0 i{background:linear-gradient(135deg,#5b6cff,#8b5cf6)}.material.c0 .glow{background:#8b5cf6}
.material.c1 i{background:linear-gradient(135deg,#22c1ee,#3b82f6)}.material.c1 .glow{background:#22c1ee}
.material.c2 i{background:linear-gradient(135deg,#22c58f,#5ad1b0)}.material.c2 .glow{background:#22c58f}
...
.metric-details span{display:flex;align-items:center;gap:5px;font-size:10px;font-weight:700;color:#3b4468}
.metric-details svg{width:12px;height:12px;color:var(--muted)}
.metric.blue .metric-top i{background:linear-gradient(135deg,#5b6cff,#22c1ee)}.metric.blue .metric-top strong{color:#4a5bff}
...
.pending{display:flex;align-items:center;gap:10px;width:100%;margin-top:6px;padding:12px 14px;border:1px solid rgba(91,108,255,.18);border-radius:18px;background:linear-gradient(135deg,rgba(91,108,255,.08),rgba(34,193,238,.08));color:var(--ink);font-weight:700;font-size:12.5px;text-align:left;transition:transform .2s,box-shadow .2s}
.pending:not(:disabled):hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(91,108,255,.18)}
...
.pending span{flex:1}
.pending b{min-width:26px;height:26px;padding:0 8px;display:grid;place-items:center;border-radius:99px;background:var(--indigo);color:#fff;font-size:11px}
.pending>svg{width:18px;height:18px;color:var(--indigo)}
...
.loading-card:hover{transform:translateY(-4px);box-shadow:0 20px 44px rgba(64,84,150,.16)}
.loading-card header{display:flex;align-items:center;justify-content:space-between;gap:8px}
.loading-card header b{display:flex;align-items:center;gap:6px;font-size:12.5px;font-weight:800;letter-spacing:.2px}
.loading-card header b svg{width:15px;height:15px;color:var(--indigo)}
.loading-card em{padding:4px 9px;border-radius:99px;font-size:9px;font-weight:800;font-style:normal;letter-spacing:.3px}
.loading-card em.stonerate{background:rgba(91,108,255,.12);color:var(--indigo)}
.loading-card em.manual{background:rgba(139,92,246,.12);color:#7c3aed}
.loading-card h3{margin:10px 0 2px;font-size:14px;font-weight:800;letter-spacing:-.2px}
.loading-card strong{display:block;font-size:20px;font-weight:900;letter-spacing:-.6px;color:var(--ink)}
.loading-card strong small{font-size:11px;font-weight:700;color:var(--muted);letter-spacing:0}
...
.loading-card footer span{display:flex;align-items:center;gap:5px;font-size:10.5px;font-weight:700;color:#3b4468}
.loading-card footer svg{width:12px;height:12px;color:var(--muted)}
.status{position:absolute;right:14px;bottom:14px;padding:5px 10px;border-radius:99px;font-size:9px;font-weight:800;font-style:normal;letter-spacing:.4px;text-transform:uppercase}
.status.next{background:rgba(245,165,36,.16);color:#b26a00}
.status.waiting{background:rgba(107,117,144,.12);color:#4e5876}
.status.loading{background:rgba(34,197,143,.16);color:#0f8a63}
.status.delayed{background:rgba(255,107,138,.16);color:#c2274b}
.view-queue{display:flex;align-items:center;justify-content:center;gap:6px;width:100%;margin-top:12px;padding:13px;border:0;border-radius:18px;background:var(--grad);color:#fff;font-weight:800;font-size:13px;box-shadow:0 14px 30px rgba(91,108,255,.30);transition:transform .2s,box-shadow .2s}
.view-queue:hover{transform:translateY(-2px);box-shadow:0 18px 36px rgba(91,108,255,.38)}
...
.skeleton i{display:block;height:130px;border-radius:22px;background:linear-gradient(90deg,#eef1fa 25%,#f8faff 50%,#eef1fa 75%);background-size:200% 100%;animation:shimmer 1.4s infinite}
.skeleton>i{height:110px}
.bottom-nav{position:fixed;left:0;right:0;bottom:0;z-index:45;padding:0 14px calc(12px + env(safe-area-inset-bottom));pointer-events:none}
.nav-pill{pointer-events:auto;position:relative;max-width:520px;margin:auto;display:grid;grid-template-columns:repeat(5,1fr);align-items:end;padding:8px 6px;border-radius:28px;background:rgba(255,255,255,.78);backdrop-filter:blur(24px) saturate(170%);-webkit-backdrop-filter:blur(24px) saturate(170%);border:1px solid rgba(255,255,255,.95);box-shadow:0 18px 44px rgba(64,84,150,.20),0 1px 0 rgba(255,255,255,.9) inset}
.nav-pill button{position:relative;display:grid;justify-items:center;gap:3px;padding:6px 2px 4px;border:0;background:transparent;color:#7a83a3;border-radius:18px;transition:color .2s,transform .2s}
.nav-pill button span{width:38px;height:38px;display:grid;place-items:center;border-radius:14px;transition:background .25s,transform .25s,box-shadow .25s}
.nav-pill button svg{width:20px;height:20px}
.nav-pill button small{font-size:9.5px;font-weight:700;letter-spacing:.2px}
...
.nav-pill button.active span{width:54px;height:54px;margin-top:-30px;border-radius:20px;background:var(--grad);color:#fff;border:4px solid #f4f7fd;box-shadow:0 14px 28px rgba(91,108,255,.45)}
.nav-pill button.active svg{width:23px;height:23px}
...
.sheet{width:100%;max-width:640px;max-height:82dvh;display:flex;flex-direction:column;padding:10px 16px 20px;border-radius:28px 28px 0 0;background:linear-gradient(180deg,#ffffff,#f7f9ff);box-shadow:0 -20px 60px rgba(27,35,64,.22);animation:sheet .32s cubic-bezier(.2,.8,.2,1)}
