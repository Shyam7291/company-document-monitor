...
function SideDrawer({ open, onClose, seller, pending, callbacks, returnFocusRef }) {
  ...
  return <Overlay open={open} onClose={close} className="left-overlay"><aside className="drawer" role="dialog" aria-modal="true" aria-label="Seller navigation">
    ...
    <footer className="drawer-foot"><span>StoneRate Seller Console</span><small>v2.0 · Ocean</small></footer>
  </aside></Overlay>;
}
...
html,body,#root{margin:0;min-height:100%;font-family:"Plus Jakarta Sans",Inter,"Segoe UI",system-ui,sans-serif;background:#eafaf5;color:#0b1f2e;-webkit-font-smoothing:antialiased}
button{font:inherit;cursor:pointer}
.app{--ink:#0b1f2e;--muted:#4b6572;--line:rgba(13,148,136,.2);--teal:#0d9488;--emerald:#059669;--sky:#0284c7;--mint:#10b981;--amber:#f59e0b;--rose:#e11d48;--grad:linear-gradient(135deg,#0f766e 0%,#0d9488 50%,#0284c7 100%);--card:rgba(255,255,255,.94);--shadow:0 12px 34px rgba(15,80,95,.10),0 2px 6px rgba(15,80,95,.05);position:relative;min-height:100dvh;padding-bottom:84px;overflow-x:hidden;background:linear-gradient(180deg,#eafaf5 0%,#e3f5f2 45%,#e0eef9 100%)}
.bg-orb{position:fixed;border-radius:50%;filter:blur(70px);opacity:.65;pointer-events:none;z-index:0;animation:float 14s ease-in-out infinite alternate}
.o1{width:320px;height:320px;left:-120px;top:-80px;background:radial-gradient(circle,#99e9d4,transparent 70%)}
.o2{width:360px;height:360px;right:-140px;top:180px;background:radial-gradient(circle,#a5dcfa,transparent 70%);animation-delay:-5s}
.o3{width:300px;height:300px;left:30%;bottom:-120px;background:radial-gradient(circle,#b6f0e6,transparent 70%);animation-delay:-9s}
.app button:focus-visible{outline:3px solid rgba(13,148,136,.35);outline-offset:2px}
...
.icon{width:40px;height:40px;display:grid;place-items:center;flex:none;border:1px solid var(--line);border-radius:14px;background:rgba(255,255,255,.9);color:#1e3a4b;box-shadow:0 6px 16px rgba(15,80,95,.08);transition:transform .2s,box-shadow .2s,border-color .2s}
.icon:hover{transform:translateY(-1px);box-shadow:0 10px 22px rgba(15,80,95,.14);border-color:rgba(13,148,136,.35)}
...
.brand i{width:22px;height:22px;margin-right:5px;display:grid;place-items:center;border-radius:7px;background:var(--grad);color:#fff;box-shadow:0 6px 14px rgba(13,148,136,.35)}
...
.bell>b{position:absolute;right:-5px;top:-6px;min-width:18px;height:18px;padding:0 4px;display:grid;place-items:center;border:2px solid #fff;border-radius:99px;background:linear-gradient(135deg,#e11d48,#f97316);color:#fff;font-size:9px;font-weight:800;box-shadow:0 4px 10px rgba(225,29,72,.45);animation:pop .5s cubic-bezier(.2,1.4,.4,1)}
...
.section-title small{display:inline-block;margin-bottom:4px;padding:3px 9px;border-radius:99px;background:rgba(13,148,136,.10);color:var(--teal);font-size:9.5px;font-weight:800;letter-spacing:.8px;text-transform:uppercase}
...
.hero-card{position:relative;overflow:hidden;display:flex;align-items:center;justify-content:space-between;gap:14px;padding:20px 18px;border-radius:24px;background:var(--grad);color:#fff;box-shadow:0 18px 40px rgba(13,148,136,.30)}
...
.material:hover,.metric:hover{transform:translateY(-4px);box-shadow:0 20px 44px rgba(15,80,95,.16)}
...
.material p{display:flex;align-items:center;gap:5px;margin:10px 0 0;padding:5px 9px;width:max-content;border-radius:99px;font-size:10px;font-weight:700;background:rgba(8,40,55,.05);color:#1e3a4b}
...
.material.c0 i{background:linear-gradient(135deg,#0f766e,#14b8a6)}.material.c0 .glow{background:#14b8a6}
.material.c1 i{background:linear-gradient(135deg,#0369a1,#0ea5e9)}.material.c1 .glow{background:#0ea5e9}
.material.c2 i{background:linear-gradient(135deg,#6d28d9,#a855f7)}.material.c2 .glow{background:#a855f7}
...
.metric-details span{display:flex;align-items:center;gap:5px;font-size:10px;font-weight:700;color:#1e3a4b}
.metric-details svg{width:12px;height:12px;color:var(--muted)}
.metric.blue .metric-top i{background:linear-gradient(135deg,#0f766e,#0284c7)}.metric.blue .metric-top strong{color:#0e7490}
.metric.amber .metric-top i{background:linear-gradient(135deg,#f59e0b,#ea580c)}.metric.amber .metric-top strong{color:#c2410c}
.metric.green .metric-top i{background:linear-gradient(135deg,#059669,#16a34a)}.metric.green .metric-top strong{color:#047857}
.pending{display:flex;align-items:center;gap:10px;width:100%;margin-top:6px;padding:12px 14px;border:1px solid rgba(13,148,136,.18);border-radius:18px;background:linear-gradient(135deg,rgba(13,148,136,.08),rgba(2,132,199,.08));color:var(--ink);font-weight:700;font-size:12.5px;text-align:left;transition:transform .2s,box-shadow .2s}
.pending:not(:disabled):hover{transform:translateY(-2px);box-shadow:0 12px 26px rgba(13,148,136,.18)}
...
.pending span{flex:1}
.pending b{min-width:26px;height:26px;padding:0 8px;display:grid;place-items:center;border-radius:99px;background:var(--teal);color:#fff;font-size:11px}
.pending>svg{width:18px;height:18px;color:var(--teal)}
...
.loading-card.delayed:before{background:linear-gradient(180deg,#e11d48,#f97316)}
.loading-card:hover{transform:translateY(-4px);box-shadow:0 20px 44px rgba(15,80,95,.16)}
...
.loading-card header b svg{width:15px;height:15px;color:var(--teal)}
.loading-card em{padding:3px 8px;border-radius:99px;font-size:8px;font-weight:800;font-style:normal;letter-spacing:.3px}
.loading-card em.stonerate{background:rgba(13,148,136,.12);color:var(--teal)}
.loading-card em.manual{background:rgba(109,40,217,.12);color:#5b21b6}
...
.loading-card footer span{display:flex;align-items:center;gap:5px;font-size:9.5px;font-weight:700;color:#1e3a4b}
...
.status.next{background:rgba(245,158,11,.2);color:#9a3412}
.status.waiting{background:rgba(15,118,110,.12);color:#115e59}
.status.loading{background:rgba(34,197,143,.16);color:#0f8a63}
.status.delayed{background:rgba(255,107,138,.16);color:#c2274b}
.view-queue{display:flex;align-items:center;justify-content:center;gap:6px;width:100%;margin-top:12px;padding:13px;border:0;border-radius:18px;background:var(--grad);color:#fff;font-weight:800;font-size:13px;box-shadow:0 14px 30px rgba(13,148,136,.30);transition:transform .2s,box-shadow .2s}
.view-queue:hover{transform:translateY(-2px);box-shadow:0 18px 36px rgba(13,148,136,.38)}
.view-queue:active{transform:scale(.98)}
.view-queue svg{width:17px;height:17px}
.empty{display:grid;place-items:center;gap:6px;padding:28px 16px;border:1.5px dashed rgba(13,148,136,.25);border-radius:22px;background:rgba(255,255,255,.6);text-align:center;color:var(--muted)}
.empty svg{width:30px;height:30px;color:var(--teal)}
.empty b{color:var(--ink);font-size:13px}
.empty span{font-size:11px}
.error{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px 14px;border:1px solid rgba(225,29,72,.3);border-radius:16px;background:rgba(225,29,72,.08);color:#9f1239;font-size:12px;font-weight:600}
.error button{display:flex;align-items:center;gap:6px;padding:8px 12px;border:0;border-radius:12px;background:#e11d48;color:#fff;font-weight:700;font-size:12px}
...
.skeleton i{display:block;height:130px;border-radius:22px;background:linear-gradient(90deg,#e4f2ef 25%,#f3fbf9 50%,#e4f2ef 75%);background-size:200% 100%;animation:shimmer 1.4s infinite}
...
.nav-pill{pointer-events:auto;position:relative;max-width:520px;margin:auto;display:grid;grid-template-columns:repeat(5,1fr);align-items:end;padding:4px 6px 3px;border-radius:22px;background:rgba(255,255,255,.78);backdrop-filter:blur(24px) saturate(170%);-webkit-backdrop-filter:blur(24px) saturate(170%);border:1px solid rgba(255,255,255,.95);box-shadow:0 18px 44px rgba(15,80,95,.20),0 1px 0 rgba(255,255,255,.9) inset}
.nav-pill button{position:relative;display:grid;justify-items:center;gap:1px;padding:3px 2px 3px;border:0;background:transparent;color:#587a89;border-radius:16px;transition:color .2s,transform .2s}
...
.nav-pill button:hover{color:var(--teal)}
.nav-pill button:hover span{background:rgba(13,148,136,.10)}
.nav-pill button:active span{transform:scale(.92)}
.nav-pill button.active{color:var(--teal)}
.nav-pill button.active span{width:44px;height:44px;margin-top:-22px;border-radius:16px;background:var(--grad);color:#fff;border:3px solid #eafaf5;box-shadow:0 12px 24px rgba(13,148,136,.45)}
...
.overlay{position:fixed;inset:0;z-index:60;display:flex;background:rgba(8,40,55,.38);backdrop-filter:blur(6px);animation:fade .25s}
.left-overlay{justify-content:flex-start}
.bottom-overlay{align-items:flex-end;justify-content:center}
.drawer{display:flex;flex-direction:column;width:min(86vw,330px);height:100%;padding:18px 16px;background:linear-gradient(180deg,#ffffff,#eefaf7);box-shadow:24px 0 60px rgba(8,40,55,.22);animation:drawer .32s cubic-bezier(.2,.8,.2,1)}
...
.seller{display:flex;align-items:center;gap:12px;margin:20px 0 16px;padding:14px;border-radius:20px;background:var(--grad);color:#fff;box-shadow:0 14px 30px rgba(13,148,136,.30)}
...
.drawer nav button{display:flex;align-items:center;gap:12px;padding:12px 12px;border:1px solid transparent;border-radius:16px;background:transparent;color:#1e3a4b;font-weight:700;font-size:13.5px;text-align:left;transition:background .2s,transform .2s,border-color .2s}
.drawer nav button svg{width:19px;height:19px;color:#587a89}
.drawer nav button span{flex:1}
.drawer nav button .chev{width:16px;height:16px;opacity:.5}
.drawer nav button:hover{background:rgba(13,148,136,.08);border-color:rgba(13,148,136,.15)}
.drawer nav button.active{background:linear-gradient(135deg,rgba(13,148,136,.14),rgba(2,132,199,.12));border-color:rgba(13,148,136,.25);color:var(--teal)}
.drawer nav button.active svg{color:var(--teal)}
.drawer nav button b{min-width:22px;height:22px;padding:0 7px;display:grid;place-items:center;border-radius:99px;background:linear-gradient(135deg,#e11d48,#f97316);color:#fff;font-size:10px}
...
.sheet{width:100%;max-width:640px;max-height:82dvh;display:flex;flex-direction:column;padding:10px 16px 20px;border-radius:28px 28px 0 0;background:linear-gradient(180deg,#ffffff,#eefaf7);box-shadow:0 -20px 60px rgba(8,40,55,.22);animation:sheet .32s cubic-bezier(.2,.8,.2,1)}
.handle{display:block;width:44px;height:5px;margin:0 auto 12px;border-radius:99px;background:#c9e2de}
...
.sheet header button:not(.icon){padding:8px 12px;border:1px solid rgba(13,148,136,.25);border-radius:12px;background:rgba(13,148,136,.08);color:var(--teal);font-weight:700;font-size:11px}
...
.note:hover{transform:translateY(-2px);box-shadow:0 10px 24px rgba(15,80,95,.12)}
.note.unread{background:linear-gradient(135deg,rgba(13,148,136,.07),rgba(2,132,199,.06));border-color:rgba(13,148,136,.22)}
...
.note.manual i{background:linear-gradient(135deg,#6d28d9,#a855f7)}
.note i svg{width:18px;height:18px}
.note span{flex:1;min-width:0;display:grid;gap:2px}
.note b{font-size:13px;font-weight:800}
.note em{font-size:11.5px;font-style:normal;color:#33505f;line-height:1.4}
.note small{font-size:10px;color:var(--muted);font-weight:600}
.note u{position:absolute;right:14px;top:14px;width:9px;height:9px;border-radius:50%;background:var(--teal);box-shadow:0 0 0 4px rgba(13,148,136,.18)}
