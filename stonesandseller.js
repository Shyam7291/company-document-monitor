...
// Hosted hero image (GitHub raw URL). Replace the file on GitHub with the new image to update it.
const HERO_IMAGE = "https://raw.githubusercontent.com/shyam7291/company-document-monitor/main/stonerate-hero-truck-loading.png";
  ...
  return <div className="app"><style>{CSS}</style>
    ...
    <main className="content">
      ...
      {loading ? <div className="skeleton"><i/><div><i/><i/></div><div><i/><i/></div></div> : <>
        <section className="hero"><div className={`hero-card ${HERO_IMAGE ? "has-img" : ""}`}>{HERO_IMAGE && <div className="hero-media"><img className="hero-img" src={HERO_IMAGE} alt="" aria-hidden="true" loading="eager"/></div>}<span className="hero-glow"/><div className="hero-text"><small>{seller.yardName}</small><h1>Your yard at a glance</h1><p>Track orders, queue and loading in real time.</p><div className="hero-stat"><strong>{number(todaySummary?.totalOrders)}</strong><span>orders today</span></div></div></div></section>
...
.hero-card{position:relative;overflow:hidden;display:flex;align-items:stretch;min-height:172px;padding:18px 18px 16px;border-radius:24px;background:var(--grad);color:#fff;box-shadow:0 18px 40px rgba(91,108,255,.30)}
.hero-media{position:absolute;top:0;right:0;bottom:0;width:72%;z-index:1;pointer-events:none;-webkit-mask-image:linear-gradient(90deg,transparent 0%,rgba(0,0,0,.35) 22%,rgba(0,0,0,.8) 45%,#000 62%);mask-image:linear-gradient(90deg,transparent 0%,rgba(0,0,0,.35) 22%,rgba(0,0,0,.8) 45%,#000 62%)}
.hero-img{display:block;width:100%;height:100%;object-fit:cover;object-position:80% 60%}
...
.hero-text{position:relative;z-index:3;display:flex;flex-direction:column;align-items:flex-start;width:62%;min-width:0}
...
.hero-card p{margin:0 0 12px;font-size:12px;opacity:.92;text-shadow:0 1px 8px rgba(30,30,80,.3)}
.hero-stat{margin-top:auto;display:flex;align-items:baseline;gap:7px;padding:9px 13px;border-radius:15px;background:rgba(255,255,255,.18);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,.4);box-shadow:0 8px 22px rgba(27,35,64,.22)}
.hero-stat strong{font-size:26px;font-weight:900;letter-spacing:-1px;line-height:1}
.hero-stat span{font-size:10.5px;opacity:.95;font-weight:700}
